...
In this phase, two additional fruit were added: the enemy fruit and the decay fruit. The enemy fruit followed a set square path and would decrease the score by 1 whenever either the snake ate this fruit or the fruit itself ran into the snake's body. The Decay fruit started with an initial score increase reward of 5 when eaten, but this reward decreases by 1 every 5 steps in the training process, where this fruit respawns at a new location when it hits a score increase reward of 0. On top of the observation space of phase 1, we added an enemy fruit danger observation, which tells the snake agent if the next move of the enemy fruit results in a collision with the snake itself. Also, instead of just observing the location of one fruit, there are now three fruit locations in the observation space.
